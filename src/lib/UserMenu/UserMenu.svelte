<script lang="ts">
	export let loginRoute: string = '/login';
	export let menuPlacement: Placement = 'bottom-end';
	import { page } from '$app/stores';
	import type { Placement } from '@floating-ui/dom';
	import Avatar from './Avatar.svelte';
	import { popup } from '@skeletonlabs/skeleton';
	import type { PopupSettings } from '@skeletonlabs/skeleton';
	import Menu from './Menu.svelte';
	let userMenuSettings: PopupSettings = {
		event: 'focus-click',
		target: 'usermenu',
		placement: menuPlacement,

		closeQuery: '.btn'
	};
</script>

{#if $page.data.session}
	<div class="flex">
		<button class="btn-icon btn-icon-sm" use:popup={userMenuSettings}>
			<Avatar />
		</button>
		<div data-popup="usermenu" class="card shadow-xl p-2">
			<Menu {loginRoute} />
		</div>
	</div>
{:else}
	<a href={loginRoute} class="btn-icon btn-icon-sm variant-filled-surface">
		<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
			><path
				fill="currentColor"
				d="M7 15q1.25 0 2.125-.875T10 12q0-1.25-.875-2.125T7 9q-1.25 0-2.125.875T4 12q0 1.25.875 2.125T7 15Zm0 3q-2.5 0-4.25-1.75T1 12q0-2.5 1.75-4.25T7 6q2.025 0 3.538 1.15T12.65 10h8.375L23 11.975l-3.5 4L17 14l-2 2l-2-2h-.35q-.625 1.8-2.175 2.9T7 18Z"
			/></svg
		>
	</a>
{/if}
